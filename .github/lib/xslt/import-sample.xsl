...
<xsl:stylesheet version="3.0"
    ...
    <xsl:template match="teix:egXML" priority="999" mode="#all">
        <pre><xsl:apply-templates></xsl:apply-templates></pre>
        <xsl:variable name="container_element" select="cudl:determine-output-element-name(.,'div')"/>
    ...
    <xsl:template name="cssHook">
        ...
        <style>
            ...
            */
            ...
            code[class*="language-"],
            pre[class*="language-"], .att, .value, .ident, .eg, .val, .xmlComment {
            color: black;
            background: none;
            ...
            .exemplum div.xmlCodeChunk {margin:0}
            
            .xmlCodeChunk, .exemplum {
            background: #fafafa;
            }
            
            li span.xmlCodeChunk {
            display:block
            }
            ...
            span.prism-code.xmlCodeChunk .pre {
